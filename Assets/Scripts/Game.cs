...
public class Game : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timeSinceBlockDrop > (1.1 - (level * 0.3)))
        {
    ...
    void CreateLayerWithHole(int y, Vector2Int holePosition)
    {
        for (int x = 0; x < PLAYFIELD_SIZE_X; x++)
        {
            for (int z = 0; z < PLAYFIELD_SIZE_Z; z++)
            {
                ...
                {
                    GameObject gameObject = Instantiate(Resources.Load("Cell", typeof(GameObject))) as GameObject;
    ...
    private void RemoveLayer(int yPosition)
    {
        ...
        SetScore(score + level * (PLAYFIELD_SIZE_X - 2) * (PLAYFIELD_SIZE_Z - 2));
        rowsCompleted++;
        if (rowsCompleted > 2)
        {
            level++;
    ...
    private Vector3Int Rotate(Vector3Int rotationObject, RotationType rotationType)
    {
        ...
        {
            RotationType.XAxisCW => new Vector3Int(rotationObject.x, rotationObject.z, BLOCK_SIZE - rotationObject.y),
